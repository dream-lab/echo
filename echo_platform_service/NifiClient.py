...
class NifiClient():
    ...
    def get_connections_for_rpg(self, rpg_id, debug = False):
        connections = self.get_all_connection_id()
        adjacent_connections = []
        for i in connections:
            connection = self.get_connection(i)
            if connection['sourceGroupId'] == rpg_id or connection['destinationGroupId'] == rpg_id:
                adjacent_connections.append(i)
    ...
    def empty_connection(self, connection_id, debug = False):
        ...
        if response.status == 200 or response.status == 202:
            ...
            while not request_fulfilled:
                conn = self.connect_to_nifi()
                newendpoint = '/nifi-api/flow/connections/' + connection_id + '/status'
                conn.request('GET', newendpoint)
                response = conn.getresponse()
                ...
                if payload['connectionStatus']['aggregateSnapshot']['flowFilesQueued'] == 0:
    ...
    def create_new_output_port(self, name):
        process_group_id = self.get_process_group_id()
        endpoint = '/nifi-api/process-groups/' + process_group_id + '/output-ports'
        conn = self.connect_to_nifi()
        conn.request('POST', endpoint,
                     json.dumps(
                         {"component": {"name": name},
    ...
    def delete_output_port(self, id):
        conn = self.connect_to_nifi()
        payload = self.get_output_port(id)
        conn.request('DELETE',
                     '/nifi-api/output-ports/' + id + '?version=' + str(payload['revision']['version']) + "&clientId="+str(uuid.uuid4()))
        response = conn.getresponse()
        if response.status == 200:
            return True
        elif response.status == 409:
    ...
    def connect_remote_output_port(self, rpg_id, port_id, proc_id, relationship_name):
        return self.connect_remote_input_port(rpg_id, port_id, proc_id, relationship_name, False)
    ...
    def enable_output_port(self, port_id):
        return self.enable_input_port(port_id, False, False)
