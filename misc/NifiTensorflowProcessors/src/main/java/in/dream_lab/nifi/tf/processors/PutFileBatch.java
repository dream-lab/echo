...
public class PutFileBatch extends AbstractProcessor {
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) {
        ...
        final Path configuredRootDirPath = Paths.get(context.getProperty(DIRECTORY).evaluateAttributeExpressions(flowFile).getValue()+"/"+batchID);
        ...
        try {
            ...
            if (batchCounterMap.containsKey(batchID)) {
                int count = batchCounterMap.get(batchID);
                ++count;
                if (count >= Integer.parseInt(context.getProperty(BATCH_SIZE).getValue())) {
                    // send the flowfile to the next guy
                    session.transfer(flowFile, REL_SUCCESS);
