...
public class GetFileFromAttribute extends AbstractProcessor {
...
//            .build();
    ...
    private final AtomicReference<FileFilter> fileFilterRef = new AtomicReference<FileFilter>();

    private final BlockingQueue<File> fileQueue = new LinkedBlockingQueue<File>();
    private final Set<File> inProcess = new HashSet<File>();    // guarded by queueLock
    private final Set<File> recentlyProcessed = new HashSet<File>();    // guarded by queueLock
...
//        properties.add(DIRECTORY);
    ...
    public void onScheduled(final ProcessContext context,String flowfile) {
        fileFilterRef.set(createFileFilter(context,flowfile));
        fileQueue.clear();
    }
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        FlowFile ipflowfile = session.get();
        String dir = null; 
        if(ipflowfile != null) {
        	dir=ipflowfile.getAttribute("Directory");
        	session.remove(ipflowfile);
        }
        if(dir != null) {
		onScheduled(context, dir);
    	final File directory =new File(dir); //new File(context.getProperty(DIRECTORY).evaluateAttributeExpressions().getValue());
        ...
        final List<File> files = new ArrayList<File>(batchSize);
        queueLock.lock();
        try {
